...
namespace BHN01.Dll
{
    [AutoTestClassAttribute("BHN01 BCU配置", "设备类型,波特率250/500,设备索引号,通道号")]
    public partial class BHN01Config : Form,IDeviceControl
    {
        ...
        public byte OneKeySetResult, OneKeyReadResult;
        public bool OneKeySetFinish, OneKeyReadFinish;

        string path;
        uint devicetype, baudrate, deviceIndex, channel;

        int TimerTickTimes = 0;


        public void OnFormClose()
        {
            ProjectGlobal.zlgCan.Can_Close();
            this.Close();
        }
        ...
        public byte  CanOpen(uint devicetype, uint baudrate, uint deviceIndex, uint channel)
        {
            ProjectGlobal.zlgCan = new ZlgCanClass(devicetype, baudrate, deviceIndex, channel);
            if ((devicetype == 2) || (devicetype == 4) || (devicetype == 14))
            {
                ProjectGlobal.zlgCan.Can_Init1();
            }
            else 
            {
                ProjectGlobal.zlgCan.Can_Init();
        ...
        private void Config_Load(object sender, EventArgs e)
        {
            ...
            OneKeyReadFinish = false;
            OneKeySetFinish = false;
            ...
            //CanOpen(21, 500, 0, 0);
            CanOpen(devicetype, baudrate, deviceIndex, channel);
            timer1.Enabled = true;
        }
        ...
        public void Init_Class(uint devicetype1, uint baudrate1, uint deviceIndex1, uint channel1)//必须有此函数
        {
            devicetype = devicetype1;
            baudrate = baudrate1;
            deviceIndex = deviceIndex1;
            channel = channel1;
        }

        [AutoTestAttribute.DescriptionAttribute("开始配置BCU", "路径")]
        public int StartToConfig(string programPath)
        {
            path = programPath;
            timer1.Enabled = false;
  
            textPath.Text = programPath;
            this.ShowDialog();

            return 1;
        }

        [AutoTestAttribute.DescriptionAttribute("获取BCU设置结果", "结果.Out")]
        public int GetTestResult(ref int result)
        {
            if ((OneKeySetResult == 1) && (OneKeyReadResult == 1))
            {
                result = 1;
                return 1;
            }
        ...
        public BHN01Config(string pathin, uint devicetypein, uint baudratein, uint deviceIndexin, uint channelin)
        {
...
// Thread was aborted normally if (_log.IsDebugEnabled) {  _log.Debug(“Splash window was aborted normally: ” + e.Message); } }  finally {  sForm = null; } } 
        ...
        private void Config_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                ...
                if (oneKeyReadThread.ThreadState == ThreadState.Running)
                {
            ...
            for (byte times = 0; times < 5; times++)//retry times
            {
                                ...
                                if ((rc1->ID == idRecv))//&& ((rc1->Data[0]) == sendData[0]) && (rc1->Data[1] == sendData[1]))
                                {
                                    if (((rc1->Data[2]) == 0) && (rc1->Data[3] == 0))
                                    {
                                        switch (configData.setStep)
                                        {
                                            ...
                                            case (byte)BcuStep.SetPackNo:
                                                {
                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                    textConfigInfo.Text += "BCU 电池包号设置成功\r\n";
                                                    break;
                                                }
                ...
                switch (configData.setStep)//Result Control
                {
                    ...
                    case (byte)BcuStep.SetPackNo:
                        {
                            ((Button)configData.objectSender).BackColor = Color.Red;
                            textConfigInfo.Text += "BCU 电池包号设置失败\r\n";
                            break;
                        }
                    case (byte)BcuStep.SetSoh:
                        {
        ...
        unsafe private void backgroundWorkerMulti_DoWork(object sender, DoWorkEventArgs e)
        {
                                                    ...
                                                    fixed (VCI_CAN_OBJ* rc2 = &recvCanMsgArray[i])
                                                    {
                                                        if ((rc2->ID == idRecv) && (rc2->Data[2] == 0) && (rc1->Data[3] == 0))
                                                        {
                                                            ...
                                                            switch (configData.setStep)
                                                            {
                                                                ...
                                                                case (byte)BcuStep.SetCellStartNo:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 电压起始位设置成功\r\n";
                                                                        break;
                                                                    }
                                                                case (byte)BcuStep.SetBmuFusePos:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 电压起始位设置成功\r\n";
                                                                        break;
                                                                    }
                                                                case (byte)BcuStep.SetBcuVoltChanNum:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 电压起始位设置成功\r\n";
                                                                        break;
                                                                    }
                                                                case (byte)BcuStep.SetUser:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 电压起始位设置成功\r\n";
                                                                        break;
                                                                    }
                                                                case (byte)BcuStep.SetBmuStart:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 电压起始位设置成功\r\n";
                                                                        break;
                                                                ...
                                                                case (byte)BcuStep.SetTempStartNo:
                                                                    {
                                                                        ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                        //textConfigInfo.Text += "BCU 温度起始位设置成功\r\n";
                                                                        break;
                                                                    }
            ...
            switch (configData.setStep)
            {
                ...
                case (byte)BcuStep.SetCellStartNo:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU电压起始位设置失败\r\n";
                        break;
                    }
                //case (byte)BcuStep.SetBmuStart:
                //    {
                ...
                case (byte)BcuStep.SetBmuFusePos:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU SetBmuFusePos位设置失败\r\n";
                        break;
                    }
                case (byte)BcuStep.SetBcuVoltChanNum:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU 通道电压设置失败\r\n";
                        break;
                    }
                case (byte)BcuStep.SetTempStartNo:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU温度起始位设置失败\r\n";
                        break;
                    }
                case (byte)BcuStep.SetChgAh:
                    {
        ...
        private void btnWConfigVerBcu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                configData.setStep = (byte)BcuStep.SetConfigVersion;
        ...
        public byte btnOneKeySetBcu()//简单调用全部按钮事件完成一键设置功能
        {
                    ...
                    try
                    {
                        ...
                        if ((backgroundWorkerSingle.IsBusy == false) && (backgroundWorkerMulti.IsBusy == false))
                        {
                            ...
                            switch (OneKeySetBcuStepNo)
                            {
                                ...
                                case 17:
                                    {
                                        btnWPackNo.PerformClick();
                                        break;
                                    }
                                case 18:
                                    {
                                        btnSetStartCellNo.PerformClick();
                                        break;
                                    }
                                case 19:
                                    {
                                        btnSetTempStart.PerformClick();
                                        break;
                                    }
                                case 20:
                                    {
                                        btnSetBmuFusePos.PerformClick();
                                        break;
                                    }
                                case 21:
                                    {
                                        btnSetBCUVolChan.PerformClick();
                                        break;
                                    }
                                case 22:
                                    {
                                        btnSetUser.PerformClick();
                                        break;
                                    }
                                case 23:
                                    {
                                        btnWriteFuse.PerformClick();
                                        break;
                                    }
                    ...
                    try
                    {
                        ...
                        if ((backgroundWorkerSingleRead.IsBusy == false) && (backgroundWorkerMultiRead.IsBusy == false))
                        {
                            ...
                            switch (OneKeyReadBcuStepNo)
                            {
                                ...
                                case 18:
                                    {
                                        btnRPackNo.PerformClick();
                                        break;
                                    }

                                case 19:
                                    {
                                        btnRCellStart.PerformClick();
                                        break;
                                    }

                                case 20:
                                    {
                                        btnRTempStart.PerformClick();
                                        break;
                                    }

                                case 21:
                                    {
                                        btnRBMUFuse.PerformClick();
                                        break;
                                    }
                                    
                                case 22:
                                    {
                                        btnReadBCUVolChan.PerformClick();
                                        break;
                                    }
                                case 23:
                                    {
                                        ReadUser.PerformClick();
                                        break;
                                    }
                                case 24:
                                    {
                                        btnReadFuse.PerformClick();
                                        break;
                                    }
        ...
        private void btnWBatInfoBcu_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                temp = double.Parse(textWBatRealAhBcu.Text) * 1000;
            ...
            for (byte times = 0; times < 5; times++)//retry times
            {
                                ...
                                if ((rc1->ID == idRecv))//&& ((rc1->Data[0]) == sendData[0]) && (rc1->Data[1] == sendData[1]))
                                {
                                    if (((rc1->Data[2]) == 0) && (rc1->Data[3] == 0))//无故障
                                    {
                                        switch (configData.setStep)
                                        {
                                            ...
                                            case (byte)BcuStep.ReadPackNo:
                                                {
                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                    textRPackNo.Text = ((rc1->Data[4] * 0x1000000 + rc1->Data[5] * 0x10000 + rc1->Data[6] * 0x100 + rc1->Data[7])).ToString();
                                                    textConfigInfo.Text += "BCU 电池包号成功\r\n";
                                                    break;
                                                }
                ...
                switch (configData.setStep)//Result Control
                {
                    ...
                    case (byte)BcuStep.ReadPackNo:
                        {
                            ((Button)configData.objectSender).BackColor = Color.Red;
                            textConfigInfo.Text += "BCU 电池包号读取失败\r\n";
                            break;
                        }
                    case (byte)BcuStep.ReadSoh:
                        {
        ...
        unsafe private void backgroundWorkerMultiRead_DoWork(object sender, DoWorkEventArgs e)
        {
                                                        ...
                                                        if ((rc2->ID == idRecv))// && (rc2->Data[2] == 0) && (rc1->Data[3] == 0))
                                                        {
                                                            ...
                                                            if (needRecvFrameCnt >= needRecvFrameNum)//全部接收完成
                                                            {
                                                                ...
                                                                switch (configData.setStep)
                                                                {
                                                                    ...
                                                                    case (byte)BcuStep.ReadTempStartNo:
                                                                        {
                                                                            lock (this)
                                                                            {
                                                                                try
                                                                                {
                                                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                                    textConfigInfo.Text += "BCU 温度起始位读取成功\r\n";
                                                                                }
                                                                                catch (System.IndexOutOfRangeException)
                                                                                { }
                                                                            }
                                                                    ...
                                                                    case (byte)BcuStep.ReadCellStartNo:
                                                                        {
                                                                            lock (this)
                                                                            {
                                                                                try
                                                                                {
                                                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                                    textConfigInfo.Text += "BCU 电压起始位读取成功\r\n";
                                                                                }
                                                                                catch (System.IndexOutOfRangeException)
                                                                                { }
                                                                            }
                                                                    ...
                                                                    case (byte)BcuStep.ReadBmuFusePos:
                                                                        {
                                                                            lock (this)
                                                                            {
                                                                                try
                                                                                {
                                                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                                    textConfigInfo.Text += "BCU BmuFusePos读取成功\r\n";
                                                                                }
                                                                                catch (System.IndexOutOfRangeException)
                                                                                { }
                                                                            }
                                                                    ...
                                                                    case (byte)BcuStep.ReadBcuVoltChanNum:
                                                                        {
                                                                            lock (this)
                                                                            {
                                                                                try
                                                                                {
                                                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                                    textConfigInfo.Text += "BCU 通道电压数读取成功\r\n";
                                                                                }
                                                                                catch (System.IndexOutOfRangeException)
                                                                                { }
                                                                            }
                                                                    ...
                                                                    case (byte)BcuStep.ReadUser:
                                                                        {
                                                                            lock (this)
                                                                            {
                                                                                ...
                                                                                {
                                                                                    ((Button)configData.objectSender).BackColor = Color.FromArgb(192, 255, 192);
                                                                                    textConfigInfo.Text += "BCU ReadConfigVer读取成功\r\n";
                                                                                }
                                                                                catch (System.IndexOutOfRangeException)
                                                                                { }
                                                                            }
            ...
            switch (configData.setStep)
            {
                ...
                case (byte)BcuStep.ReadBmuFusePos:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU BmuFusePos数读取失败\r\n";
                        break;
                    }
                case (byte)BcuStep.ReadBcuVoltChanNum:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU 通道电压数读取失败\r\n";
                        break;
                    }
                case (byte)BcuStep.ReadUser:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU ReadConfigVer读取失败\r\n";
                        break;
                    }
                ...
                case (byte)BcuStep.ReadCellStartNo:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU 电压起始位读取失败\r\n";
                        break;
                    }
                case (byte)BcuStep.ReadTempStartNo:
                    {
                        ((Button)configData.objectSender).BackColor = Color.Red;
                        textConfigInfo.Text += "BCU 温度起始读取失败\r\n";
                        break;
                    }
        ...
        public byte ReadFile(string filePath)
        {
            ...
            if (filePath!="")
            {
                string fName = filePath;//openFileDialog1.FileName;
                textConfigPath.Text = fName;
                ...
                if (File.Exists(fName))
                {
                    ...
                    try
                    {
                        ...
                        try 
                        {
                            byte.Parse(textWBmuNumBcu.Text);
                        }
                        catch 
                        {
                            MessageBox.Show("配置文件：BMU个数异常");
                            return 0;
                        }
                        ...
                        textWBatRealAhBcu.Text = iniFile.IniReadValue(fName, "主板", "实际容量");
                        textWRateSumvBcu.Text = iniFile.IniReadValue(fName, "主板", "电池包额定总电压");
                        textWPackNo.Text = iniFile.IniReadValue(fName, "主板", "电池包组号");
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (textConfigInfo.Text.IndexOf("失败") > 0)
            {
                ...
                try
                {
                    backgroundWorkerSingle.Dispose();
                }
                catch { }

                try
                {
                    backgroundWorkerMulti.Dispose();
                }
                catch { }
            ...
            TimerTickTimes = (TimerTickTimes > 255) ? 255 : (++TimerTickTimes);
            ...
            if (TimerTickTimes == 8)
            {
                if (OneKeySetResult == 1)
                {
                    btnOneKeyReadBcu();
                }
            }

            if (OneKeyReadFinish == true)
            {
                timer1.Enabled = false;

                if (btnWSocBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWSohBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWAhBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWProNoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWCarNoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWConfigVerBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWClockBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWHeatControl.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWChginfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWCellInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWTempInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWLevelBcu1.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWLevelBcu2.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWBatInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWLevelBcu3.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWPackNo.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWChgAhBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;
                }
                if (btnWChgTimeBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnSetStartCellNo.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnSetTempStart.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnSetBmuFusePos.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                ///////////////////////////////////////
                if (btnRSocBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRSohBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRAhBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRProNoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRCarNoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRConfigVerBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRClockBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRHeatControl.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRChginfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRCellInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRTempInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRLevelBcu1.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRLevelBcu2.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRLevelBcu3.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRBatInfoBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRPackNo.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnRChgAhBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRChgTimeBcu.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnRCellStart.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;

                }
                if (btnRTempStart.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeySetResult = 0;

                }
                if (btnRBMUFuse.BackColor != Color.FromArgb(192, 255, 192))
                {
                    OneKeyReadResult = 0;
                ...
                //    //iniFile.IniWriteValue(@"D:\BlueWay\configresult.ini", "BMURESULT", "BMURESULT", "PASS");
                //}
                //else
                //{
                //    //iniFile.IniWriteValue(@"D:\BlueWay\configresult.ini", "BMURESULT", "BMURESULT", "FAIL");
            ...
            //if (TimerTickTimes >= 8)////////////超时处理
            //{

            //    if (OneKeySetResult == 0)
            //    {
            //        try
            //        {
            //            oneKeyReadThread.Abort();
        ...
        private void btnSetStartCellNo_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[70];
            ConfigData configData = new ConfigData();
            configData.retryTimes = 5;
            ...
            try
            {
                ...
                configData.setStep = (byte)BcuStep.SetCellStartNo;
        ...
        private void btnRCellStart_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 62;
            configData.cmd = 4;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadCellStartNo;
                if (backgroundWorkerMultiRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerMultiRead.RunWorkerAsync(configData);
        ...
        private void btnSetTempStart_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[70];
            ConfigData configData = new ConfigData();
            configData.retryTimes = 5;
        ...
        private void btnSetBmuFusePos_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[70];
            ConfigData configData = new ConfigData();
            configData.retryTimes = 5;
            ...
            try
            {
                ...
                configData.setStep = (byte)BcuStep.SetBmuFusePos;
        ...
        private void btnRTempStart_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 62;
            configData.cmd = 6;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadTempStartNo;
                if (backgroundWorkerMultiRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerMultiRead.RunWorkerAsync(configData);
        ...
        private void btnRBMUFuse_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 10;
            configData.cmd = 13;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadBmuFusePos;
                if (backgroundWorkerMultiRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerMultiRead.RunWorkerAsync(configData);
        ...
        private void btnWPackNo_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[8];
            ...
            try
            {
                ...
                temp = double.Parse(textWPackNo.Text);
                data[0] = (byte)(temp / 0x1000000);
                data[1] = (byte)(temp / 0x10000);
                data[2] = (byte)(temp / 0x100);
                data[3] = (byte)(temp);
                ...
                configData.setStep = (byte)BcuStep.SetPackNo;
        ...
        private void btnRPackNo_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 4;
            configData.cmd = 2;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadPackNo;
                if (backgroundWorkerSingleRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerSingleRead.RunWorkerAsync(configData);
        ...
        private void btnSetBCUVolChan_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[70];
            ConfigData configData = new ConfigData();
            configData.retryTimes = 5;
            ...
            try
            {
                ...
                configData.setStep = (byte)BcuStep.SetBcuVoltChanNum;
        ...
        private void btnReadBCUVolChan_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 20;
            configData.cmd = 3;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadBcuVoltChanNum;
                if (backgroundWorkerMultiRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerMultiRead.RunWorkerAsync(configData);
        ...
        private void btnSetConfigVer_Click(object sender, EventArgs e)
        {
            byte[] data = new byte[70];
            ConfigData configData = new ConfigData();
            configData.retryTimes = 5;
            ...
            try
            {
                ...
                configData.setStep = (byte)BcuStep.SetUser;
        ...
        private void ReadConfigVer_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 8;
            configData.cmd = 10;
            ...
            try
            {
                configData.setStep = (byte)BcuStep.ReadUser;
                if (backgroundWorkerMultiRead.IsBusy == false)
                {
                    ((Button)sender).BackColor = Color.Gray;
                    backgroundWorkerMultiRead.RunWorkerAsync(configData);
        ...
        private void btnReadFuse_Click(object sender, EventArgs e)
        {
            ConfigData configData = new ConfigData();
            configData.len = 1 + 20;
            configData.cmd = 3;
